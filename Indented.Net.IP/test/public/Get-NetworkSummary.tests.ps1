InModuleScope Indented.Net.IP {
    Describe 'Get-NetworkSummary' {
        ...
        It 'Identifies 10/8 as a private range' {
            (Get-NetworkSummary 10/8).IsPrivate | Should Be $true
            (Get-NetworkSummary 10.0.0.0/8).IsPrivate | Should Be $true
            (Get-NetworkSummary 10.0.0.0 255.0.0.0).IsPrivate | Should Be $true
        }

        It 'Identifies 172.16/12 as a private range' {
            (Get-NetworkSummary 172.16/12).IsPrivate | Should Be $true
            (Get-NetworkSummary 172.16.0.0/12).IsPrivate | Should Be $true
            (Get-NetworkSummary 172.16.0.0 255.240.0.0).IsPrivate | Should Be $true
        }

        It 'Identifies 192.168/16 as a private range' {
            (Get-NetworkSummary 192.168/16).IsPrivate | Should Be $true
            (Get-NetworkSummary 192.168.0.0/16).IsPrivate | Should Be $true
            (Get-NetworkSummary 192.168.0.0 255.255.0.0).IsPrivate | Should Be $true
        }
        ...
        It 'Has valid examples' {
            (Get-Help Get-NetworkSummary).Examples.Example.Code | ForEach-Object {
